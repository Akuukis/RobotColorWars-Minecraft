...
Nav = {
  ...
  GetPath = function(self,tx,tz)  -- Based on code (Sep 21, 2006) by Altair who ported and upgraded code of LMelior
  --[[ PRE:
...
--  local ysize=table.maxn(Nav.Map)					-- vertical map size
  ...
  while openk>0 do   -- Growing loop
    ...
    printf("(%s,%s)",curbase.x, curbase.z)
    ...
    local suc=true
    local id=0
    suc,id = pcall(function() if Nav.Map[curbase.x+1][curbase.z].Id==0 then return 0 else return 1 end end)
    if suc~=true then NorhtOK = true elseif id == 0 then NorthOK = true else NorthOK = false end
    suc,id = pcall(function() if Nav.Map[curbase.x][curbase.z+1].Id==0 then return 0 else return 1 end end)
    if suc~=true then EastOK = true elseif id == 0 then EastOK = true else EastOK = false end
    suc,id = pcall(function() if Nav.Map[curbase.x-1][curbase.z].Id==0 then return 0 else return 1 end end)
    if suc~=true then SouthOK = true elseif id == 0 then SouthOK = true else SouthOK = false end
    suc,id = pcall(function() if Nav.Map[curbase.x][curbase.z-1].Id==0 then return 0 else return 1 end end)    
    if suc~=true then WestOK = true elseif id == 0 then WestOK = true else WestOK = false end
    --[[
    ...
    if closedlist[closedk].x==tx and closedlist[closedk].z==tz then
      ...
      for i=table.maxn(pathIndex),1,-1 do table.insert(path,{x=closedlist[pathIndex[i]].x, z=closedlist[pathIndex[i]].z}); printf("(%s,%s)", path[table.maxn(pathIndex)-i+1].x, path[table.maxn(pathIndex)-i+1].z) end
      closedlist=nil
  ...
  GoPath = function(self,tx,tz,tries)
    ...
    local j=1
    repeat
      if Nav.X==tx and Nav.Z==tz then return i else tries=tries-1 end
      printfs("Nav.GoPath() New path! Tries left:%s\n",tries)
      local fpath = Nav:GetPath(tx,tz)
      if 0 == table.maxn(fpath) then return i end
      if fpath == nil then return nil else j=0 end
      repeat
        j=j+1 
  ...
  UpdateMap = function(self,location,value) -- location{nil|dir|XZY), value{0=air,1=unknown block,1+=known block}  
    ...
    if type(location)=="nil" then
      ...
      Nav.Map[x][z].Updated = os.time()
      if turtle.detect() then Nav.Map[x][z].Id = 1 else Nav.Map[x][z].Id = 0 end
      elseif type(location)=="number" then
      x,z,y = Nav:DirToCoord(location)
      ...
      if location~=5 then
        ...
        Nav:ExploreMap(x,z,y)
        Nav.Map[x][z].Updated = os.time()
        if turtle.detectUp() then Nav.Map[x][z].Id = 1 else Nav.Map[x][z].Id = 0 end
        end
      if location~=4 then
      ...
      x,z,y = location.x, location.z, location.y
      if value == nil then value = 1 end
      Nav:ExploreMap(x,z,y)
      Nav.Map[x][z].Updated = os.time()
      Nav.Map[x][z].Id = value
      end
    end,
...
--[[
  for x=math.max(startx,x)+2,math.min(startx,x)-2,-1 do
    for z=math.min(startz,z)-2,math.max(startz,z)+2,1 do
      if Nav.Map[x]==nil then
        printf(".")
        elseif Nav.Map[x][z]==nil then
        printf(".")
